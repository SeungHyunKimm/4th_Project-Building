...
namespace Microsoft.MixedReality.Toolkit.UI
{
    ...
    public class ObjectManipulator_Star : MonoBehaviour, IMixedRealityPointerHandler, IMixedRealityFocusChangedHandler, IMixedRealitySourcePoseHandler
    {
        #region Public Enums
        ...
        private TransformFlags twoHandedManipulationType = TransformFlags.Move | TransformFlags.Rotate | TransformFlags.Scale
            //ㅂ수정중
            | TransformFlags.Scale_x | TransformFlags.Scale_y | TransformFlags.Scale_z
            ;
        ...
        }   public void OnClickScaleY() {
            if (TwoHandedManipulationType != TransformFlags.Scale_y) TwoHandedManipulationType = TransformFlags.Scale_y;
            else TwoHandedManipulationType = (TransformFlags)0;
        }   public void OnClickScaleZ() {
            if (TwoHandedManipulationType != TransformFlags.Scale_z) TwoHandedManipulationType = TransformFlags.Scale_z;
            else TwoHandedManipulationType = (TransformFlags)0;
        }
        [SerializeField]
        ...
        private void ApplyTargetTransform(MixedRealityTransform targetTransform)
        {
            ...
            if (rigidBody == null)
            {
                ...
                //ㅂ수정중
                if (!transformUpdated.HasFlag(TransformFlags.Scale_x) 
                    || !transformUpdated.HasFlag(TransformFlags.Scale_y)
                    || !transformUpdated.HasFlag(TransformFlags.Scale_z)
                    )
                {
                    HostTransform.localScale = applySmoothing ? smoothingLogic.SmoothScale(HostTransform.localScale, targetTransform.Scale, scaleLerpTime, Time.deltaTime) : targetTransform.Scale;
                }
